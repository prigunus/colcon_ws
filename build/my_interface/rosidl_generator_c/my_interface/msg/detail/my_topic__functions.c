// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from my_interface:msg/MyTopic.idl
// generated code does not contain a copyright notice
#include "my_interface/msg/detail/my_topic__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"


// Include directives for member types
// Member `stamp`
#include "builtin_interfaces/msg/detail/time__functions.h"

bool
my_interface__msg__MyTopic__init(my_interface__msg__MyTopic * msg)
{
  if (!msg) {
    return false;
  }
  // stamp
  if (!builtin_interfaces__msg__Time__init(&msg->stamp)) {
    my_interface__msg__MyTopic__fini(msg);
    return false;
  }
  // my_float
  return true;
}

void
my_interface__msg__MyTopic__fini(my_interface__msg__MyTopic * msg)
{
  if (!msg) {
    return;
  }
  // stamp
  builtin_interfaces__msg__Time__fini(&msg->stamp);
  // my_float
}

bool
my_interface__msg__MyTopic__are_equal(const my_interface__msg__MyTopic * lhs, const my_interface__msg__MyTopic * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // stamp
  if (!builtin_interfaces__msg__Time__are_equal(
      &(lhs->stamp), &(rhs->stamp)))
  {
    return false;
  }
  // my_float
  if (lhs->my_float != rhs->my_float) {
    return false;
  }
  return true;
}

bool
my_interface__msg__MyTopic__copy(
  const my_interface__msg__MyTopic * input,
  my_interface__msg__MyTopic * output)
{
  if (!input || !output) {
    return false;
  }
  // stamp
  if (!builtin_interfaces__msg__Time__copy(
      &(input->stamp), &(output->stamp)))
  {
    return false;
  }
  // my_float
  output->my_float = input->my_float;
  return true;
}

my_interface__msg__MyTopic *
my_interface__msg__MyTopic__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  my_interface__msg__MyTopic * msg = (my_interface__msg__MyTopic *)allocator.allocate(sizeof(my_interface__msg__MyTopic), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(my_interface__msg__MyTopic));
  bool success = my_interface__msg__MyTopic__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
my_interface__msg__MyTopic__destroy(my_interface__msg__MyTopic * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    my_interface__msg__MyTopic__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
my_interface__msg__MyTopic__Sequence__init(my_interface__msg__MyTopic__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  my_interface__msg__MyTopic * data = NULL;

  if (size) {
    data = (my_interface__msg__MyTopic *)allocator.zero_allocate(size, sizeof(my_interface__msg__MyTopic), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = my_interface__msg__MyTopic__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        my_interface__msg__MyTopic__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
my_interface__msg__MyTopic__Sequence__fini(my_interface__msg__MyTopic__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      my_interface__msg__MyTopic__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

my_interface__msg__MyTopic__Sequence *
my_interface__msg__MyTopic__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  my_interface__msg__MyTopic__Sequence * array = (my_interface__msg__MyTopic__Sequence *)allocator.allocate(sizeof(my_interface__msg__MyTopic__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = my_interface__msg__MyTopic__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
my_interface__msg__MyTopic__Sequence__destroy(my_interface__msg__MyTopic__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    my_interface__msg__MyTopic__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
my_interface__msg__MyTopic__Sequence__are_equal(const my_interface__msg__MyTopic__Sequence * lhs, const my_interface__msg__MyTopic__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!my_interface__msg__MyTopic__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
my_interface__msg__MyTopic__Sequence__copy(
  const my_interface__msg__MyTopic__Sequence * input,
  my_interface__msg__MyTopic__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(my_interface__msg__MyTopic);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    my_interface__msg__MyTopic * data =
      (my_interface__msg__MyTopic *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!my_interface__msg__MyTopic__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          my_interface__msg__MyTopic__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!my_interface__msg__MyTopic__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
